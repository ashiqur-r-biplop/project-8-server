...
async function run() {
  try {
    ...
    // user-feedback posting(arif)
    app.post("/user-feedback", async (req, res) => {
      const { name, photo, rating, feedbackMessage, ticketId } = req.body;
      ...
      // don't remove this below code(Arif)

      // const query = { _id: new ObjectId(ticketId) };
      // const result = await ticketsCollection.findOne(query);

      // result.feedbackSent = "done";
      // // Update the document in the same collection
      // await ticketsCollection.updateOne(query, { $set: result });

      try {
        const response = await feedbackCollection.insertOne(feedback);
